...
export abstract class HttpService extends HttpServiceBaseService {
  ...
  get<T>(URL_Config: UrlConfig): Observable<T> {
    return this.http.get<ApiResponse<T>>(`${this.domainName}${this.baseUrl}${URL_Config.apiName}`, { params: URL_Config.params }).pipe(map(event => {
      return event.data;
    }));
  }
  ...
  postFilter<T, D>(URL_Config: UrlConfig, body: T): Observable<D> {
    return this.http.post<ApiResponse<D>>(`${this.domainName}${this.baseUrl}${URL_Config.apiName}`, body, { params: URL_Config.params }).pipe(map(event => {
  ...
  put<T, D>(URL_Config: UrlConfig, body: T): Observable<T> {
    return this.http.put<ApiResponse<D>>(`${this.domainName}${this.baseUrl}${URL_Config.apiName}`, body, { params: URL_Config.params }).pipe(map((event: any) => {
  ...
  delete(URL_Config: UrlConfig, id: any): Observable<boolean> {
    return this.http.delete<ApiResponse<boolean>>(`${this.domainName}${this.baseUrl}${URL_Config.apiName}`, { body: id, params: URL_Config.params }).pipe(map((event: any) => {
      this.alertHandling(event);
      return event.data;
    }));
  }
